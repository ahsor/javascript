...
<html>
  <body>
    <ul>
      ...
      console.log( $elems );
      // HTMLCollection { 0: li#apple, 1: li#banana, 2: li#orange, length: 3, … } 출력
      // 즉 배열은 아니고 객체로 반환
...
<html>
  <body>
    ...
    <script>
      ...
      [...$lisFromDocument].forEach(element => {
...
<html>
  <body>
    ...
    <script>
      ...
      // 전달된 class 값을 갖는 요소가 존재하지 않는 경우  빈HTMLCollection을 반환, null 아님
...
1. 요소를 만족시키는 요소 노드가 여러 개인 경우 첫번째 요소 노드만 반환 
2. 요소를 만족시키는 요소 노드가 존재하지 않는 경우 null 반환
3. 선택자가 문법에 맞지 않은 경우 DOMException 에러 발생
```
...
<html>
  <body>
    ...
    <script>
	  ...
	  // 즉, css 선택자를 만족시키는 모든 요소를 탐색하고 반환
      const $elems = document.querySelectorAll('ul > li');
      // 취득한 요소 노드들은 DOMCollection객체인 NodeList 객체에 담겨 반환된다. 즉 배열 반환
	  // 아이디, 클래스, 태그와 달리  HTMLCollection 객체를 반환하지 않음 
...
<html>
  ...
  <script>
	...
	// 일치하는 요소가 있다면 에트리뷰트 변경 
    if( $apple.matches('#fruits > li.apple') ){
        document.querySelector('#fruits > li.apple').style.color = "red";
    }
  </script>
</html>
```

# HTMLCollection과 NodeList 특징
# 1. HTMLCollection은 노드 객체의 상태 변화를 실시간으로 반영
# 2. NodeList는 대부분은 실시간이 아니지만 실시간으로 동작할 때가 있다. 
# 39-18
...
<html>
  <body>
    ...
    <script>
      // getElementsByClassName()과 getElementsByTagName()은 노드 객체의 상태변화를 실시간으로 반영하는 살아 있는 DOM 콜렉션
      ...
      for (let i = 0; i < $elems.length; i++) {
        ...
        // 디버깅을 이용하여 i의 변화를 살펴 보면 
        // 실시간으로 변경되므로 li:nth-child(2)은 변경되지 않음을 확인할 수 있다. 
		//39-19 // for 문을 역방향으로 순회하므로서 이러한 문제를 회피할 수 있다.
		//39-20또한 while로 무한 반복하여 회피하거나
		//39-21 고차함수를 이용하여 회피한다. 
      }
...
```javascript
// // for 문을 역방향으로 순회하므로서 이러한 문제를 회피할 수 있다.
...
```javascript
// while 문으로 HTMLCollection에 요소가 남아 있지 않을 때까지 무한 반복하여 회피할 수 있다.
let i = 0;
...
```javascript
// .getElementsByClassName('red'); 를 다음으로 대치하면 
...
// querySelectorAll은 정적인 NodeList를 반환하지만 
// 다음의  childNodes가 반환하는 NodeList 객체는 HTMLCollection객체와 같이 실시간으로 상태 반영하므로 주의 
```

# 39-23
# childNodes가 반환하는 NodeList 객체는 HTMLCollection객체와 같이 실시간으로 상태 반영
...
<html>
  ...
  <script>
    ...
    for (let i = 0; i < childNodes.length; i++) {
	  ...
	  // 위의 회피 전략 필요 for (let i = childNodes.length-1; i >= 0; i--) 
...
# 39-24
# 39-28 예제부터 자식 노드 순회에 대한 언급을 사용 
```html
<!DOCTYPE html>
<html>
  <body>
...
```
```
# childNodes 대신 children을 사용해도 마찬가지 문제 발생 
  <script>
    const $fruits = document.getElementById('fruits');
    ...
    // 예상과 다르게 $fruits 요소의 모든 자식 노드가 삭제되지 않는다.
    console.log(children); // NodeList(2) [li, li]
  </script>
```
...
<html>
  ...
  <script>
    ...
    // 자식 노드 중에 텍스트 노드가 아닌 요소 노드가 존재하는지는 확인한다.
     ...
     console.log(!!$fruits.children.length); // 0 -> !0 === true -> !true -> false
	// p112 : NaN, Undefined, 0, Null == false 암묵적 타입변환 
    // 자식 노드 중에 텍스트 노드가 아닌 요소 노드가 존재하는지는 확인한다.
